...
class AsigCategoriaModel extends Model{
    ...
    public function obtenerListaAsigCategoriabyFinca($finca) {
        $sql = "SELECT cat_lote.id_lot_cat, lote.nombre AS lote, lote.id_lote, categoria.categoria AS categoria, categoria.produce, categoria.id_cat, cat_lote.cantidad FROM cat_lote INNER JOIN lote ON cat_lote.lote = lote.id_lote INNER JOIN categoria ON cat_lote.categoria = categoria.id_cat WHERE lote.finca = ? ORDER BY cat_lote.id_lot_cat DESC";
